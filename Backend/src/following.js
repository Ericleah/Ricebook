...
async function removeFromFollowing(req, res) {
  ...
  try {
    const userToUnfollow = await User.findOne({ username: usernameToUnfollow }, "_id");
    if (!userToUnfollow) {
    ...
    const updatedUser = await User.findByIdAndUpdate(
      ...
      { new: true }
    ).populate("following", "username"); // Ensure `following` is updated with populated usernames

    if (!updatedUser) {
      return res.status(404).send({ error: "Logged in user not found" });
    }

    // Send updated following list
    res.status(200).send({
      username: req.session.user.username,
      following: updatedUser.following.map((user) => user.username),
    });
  } catch (error) {
